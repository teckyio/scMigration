...
SELECT 'SQUATTER_USE & SQUATTER_MATERIAL_PRO' AS "TABLE_NAME",
       (SELECT COUNT(*) FROM SDE_SQ.SQUATTERUSE UNION ALL SELECT OBJECTID FROM SDE_SQ.SQUATTERUSE_PRO) AS "TOTAL",
       (SELECT COUNT(*) FROM SQ.SQUATTER_USES s WHERE s.OBJECT_ID IS NOT NULL AND s.OBJECT_ID IN (SELECT OBJECTID FROM SDE_SQ.SQUATTERUSE)) AS "SUCCESS"
FROM dual
UNION ALL
SELECT 'SQUATTER_MATERIAL & SQUATTER_MATERIAL_PRO' AS "TABLE_NAME",
       (SELECT COUNT(*) FROM SDE_SQ.SQUATTERMATERIAL UNION ALL SELECT OBJECTID FROM SDE_SQ.SQUATTERMATERIAL_PRO) AS "TOTAL",
       (SELECT COUNT(*) FROM SQ.SQUATTER_MATERIALS s WHERE s.OBJECT_ID IS NOT NULL AND s.OBJECT_ID IN (SELECT OBJECTID FROM SDE_SQ.SQUATTERMATERIAL)) AS "SUCCESS"
FROM dual
UNION ALL
