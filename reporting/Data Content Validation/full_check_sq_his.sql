TRUNCATE TABLE data_validation_squatter_histories;
DECLARE quantity INT := 481;
table_count INT;
v_dlo_id VARCHAR2(36);
BEGIN -- Insert data into the table with validation checks
INSERT INTO data_validation_squatter_histories (objectid, target_ObjectId, error_msg, is_valid)
SELECT rec.objectid AS objectid,
	...
	RTRIM(
		...
		END || CASE
			...
			ELSE 'SQUATTER_ID;'
		END || CASE
			WHEN rec.DIMENSION_L = s.dimensions_l THEN ''
			ELSE 'DIMENSIONS_L;'
		END || CASE
			WHEN rec.DIMENSION_B = s.dimensions_b THEN ''
			ELSE 'DIMENSIONS_B'
		END || CASE
			WHEN rec.DIMENSION_H = s.dimensions_h THEN ''
			ELSE 'DIMENSIONS_H'
		END || CASE
		...
		END || CASE
			WHEN rec.BOOKNO = s.book_no THEN ''
			ELSE 'BOOK_NO;'
		END || CASE
			WHEN rec.SERIALNO = s.serial_no THEN ''
			ELSE 'SERIAL_NO;'
		END || CASE
			WHEN rec.SURVEYNO = s.survey_no THEN ''
			ELSE 'SURVEY_NO;'
		END || CASE
			WHEN rec.FILEREF = s.case_reference THEN ''
			ELSE 'CASE_REFERENCE;'
		END || CASE
		...
		END || CASE
			...
			ELSE 'SC_OFFICE;'
		END || CASE
			WHEN rec.SURVEYNOPREFIX = s.survey_no_prefix THEN ''
			ELSE 'SURVEY_NO_PREFIX;'
		END || CASE
			WHEN rec.HASREMARK = s.has_remark THEN ''
			ELSE 'HAS_REMARK;'
		END || CASE
			WHEN NVL(rec.HOUSENO, -1) = NVL(s.house_no, -1) THEN ''
			ELSE 'HOUSE_NO;'
		END || CASE
			WHEN rec.DISPLAYNAME = s.display_name THEN ''
			ELSE 'DISPLAY_NAME;'
		END || CASE
		...
		END || CASE
			...
			ELSE 'UNITS;'
		END || CASE
			WHEN rec.SERIALNO_EDIT = s.serial_no_edit THEN ''
			ELSE 'SERIAL_NO_EDIT;'
		END || CASE
			WHEN NVL(
				rec.RECORDDATE_EDIT,
	...
	CASE
		...
		ELSE 0
	END AS is_valid
FROM SDE_SQ.SQUATTER_HIS rec
	LEFT JOIN SQ.SQUATTER_HISTORIES s ON rec.OBJECTID = s.object_id
WHERE quantity = 0
	OR ROWNUM <= quantity;
-- Commit transaction
COMMIT;
